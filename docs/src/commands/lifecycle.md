...
Lifecycle commands and default inspection target records in the canonicalized
current project directory. The readable subcommand forms are:
...
The interface uses `--scope SCOPE` rather than separate boolean flags
...
before the record name. JSON output always retains the complete project key. The
`current` response keeps its `data` array shape; `subtree` and `global` responses
use `data.scope` and `data.records` so the scope is explicit to callers.
