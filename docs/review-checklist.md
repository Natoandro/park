...
- [x] `ps --scope` accepts only the documented enum, defaults to the exact current project, applies path-boundary-safe subtree filtering, and makes non-current human listings unambiguous.
