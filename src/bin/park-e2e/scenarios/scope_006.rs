...
pub fn preserve_non_utf8_names_and_arguments() -> Result<(), String> {
    ...
    expect_success("non-UTF-8 argument launch", &run(&environment, &launch_arguments)?)?;
    ...
    expect_success("wait for non-UTF-8 argument command", &run(&environment, &wait_arguments)?)?;
    ...
    expect_success("status for non-UTF-8 argument command", &status)?;
    let record = parse_json("non-UTF-8 argument status", &status)?
        .get("data")
        .cloned()
    ...
    {
        return Err(format!("status changed the ASCII name: {record}"));
    }
    ...
    let actual_arguments = record
        ...
        .ok_or_else(|| "non-UTF-8 argument status has no arguments".to_owned())?
        .iter()
        .map(|argument| argument.as_str().unwrap_or_default().to_owned())
        .collect::<Vec<_>>();
    if actual_arguments != expected_arguments {
        return Err(format!(
            "status changed non-UTF-8 argument bytes: expected {expected_arguments:?}, got {actual_arguments:?}"
        ));
    }
    ...
    expect_success("first non-UTF-8 argument logs", &first_logs)?;
    if !first_logs.stdout.windows(2).any(|bytes| bytes == b"80")
        || !first_logs.stdout.windows(2).any(|bytes| bytes == b"ff")
    {
        return Err(format!(
            "command did not receive the original argument bytes: {:?}",
            String::from_utf8_lossy(&first_logs.stdout)
        ));
    }

    let restart_arguments = vec![OsString::from("restart"), name.clone()];
    expect_success("restart non-UTF-8 argument command", &run(&environment, &restart_arguments)?)?;
    expect_success("wait after non-UTF-8 argument restart", &run(&environment, &wait_arguments)?)?;
    let second_logs = run(&environment, &logs_arguments)?;
    expect_success("second non-UTF-8 argument logs", &second_logs)?;
    let expected_logs = [first_logs.stdout.as_slice(), first_logs.stdout.as_slice()].concat();
    if second_logs.stdout != expected_logs {
        return Err("restart did not preserve the non-UTF-8 argument bytes".to_owned());
    }
    Ok(())
}
...
fn run(
    ...
    command
        .output()
        .map_err(|error| format!("execute non-UTF-8 argument Park command: {error}"))
}
